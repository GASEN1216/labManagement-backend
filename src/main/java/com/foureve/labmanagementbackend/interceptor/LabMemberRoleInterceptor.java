...
@Component
@Slf4j
public class LabMemberRoleInterceptor implements HandlerInterceptor {
    @Resource
    private UserDao userDao;
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        Long userId = RequestHolder.get().getUserId();
        if (!Objects.equals(userDao.getById(userId).getRole(), RoleEnum.LAB_MEMBER.getCode())&& !Objects.equals(userDao.getById(userId).getRole(), RoleEnum.ADMIN.getCode())){
